...
let make t =
  ...
  Pm_file.write_lines maf_file_list [Fileutils.join [t.tmp_dir; "profile.maf"]] >>= fun () ->
  Pm_file.write_lines untranslate_file_list [profiles_left; profiles_right]     >>= fun () ->
  let cmds =
    [ Printf.sprintf
