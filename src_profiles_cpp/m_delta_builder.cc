...
namespace Para_mugsy {
  M_option<M_delta_entry> M_delta_builder::to_delta() const {
    if(ref_start != ref_pos && query_start != query_pos) {
      M_profile_idx query_start = query_metaprofile.profile_idx_of_profile_idx(query_start);
      M_profile_idx query_pos = query_metaprofile.profile_idx_of_profile_idx(query_pos - 1);
